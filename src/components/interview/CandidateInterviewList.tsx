    ...
    return (
      <Card className="p-8 text-center">
        ...
        <p className="text-gray-500">
          Recommended candidates from the agent evaluation will appear here for AI interviews.
        </p>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {candidates.map((candidate) => {
        // Get the job ID from the recommended candidates
        const recommendedCandidate = agentCommunicationService.getRecommendedCandidates()
          .find(rc => rc.candidateId === candidate.id);
        const jobId = recommendedCandidate?.jobId || "default-job-id";
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  ...
                  {recommendedCandidate && (
                    <p><strong>Recommendation Score:</strong> {recommendedCandidate.score}</p>
                  )}
                  {candidate.interview?.verdict && (
                    <p><strong>Interview Result:</strong> {candidate.interview.verdict}</p>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-2">
                {!candidate.interview || candidate.interview.stage === 'pending' ? (
                  <Button
                    onClick={() => onStartInterview(candidate.id, jobId)}
